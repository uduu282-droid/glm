...
class ChatSession:
    ...
    def _complete(self, message: str, user_msg_id: str, parent_comp_id: str | None) -> str:
        """Send completion request with retry logic for rate limits."""
        max_retries = 3
        retry_delay = 3
        
        for attempt in range(max_retries):
            try:
                return self._complete_internal(message, user_msg_id, parent_comp_id)
            except RuntimeError as e:
                if "RATE_LIMITED" in str(e) and attempt < max_retries - 1:
                    print(f"\n\033[33m⏳ Rate limited, retrying in {retry_delay}s... (attempt {attempt + 1}/{max_retries})\033[0m", flush=True)
                    time.sleep(retry_delay)
                    retry_delay *= 2  # exponential backoff
                    continue
                raise

    def _complete_internal(self, message: str, user_msg_id: str, parent_comp_id: str | None) -> str:
        self.turn += 1
        ts_ms      = _ms()
        request_id = _uid()
        ...
        def _cb(chunk: bytes) -> None:
            ...
            while "\n" in buf[0]:
                ...
                if "error" in data:
                    ...
                    if data["error"].get("code") == "MODEL_CONCURRENCY_LIMIT":
                        rate_limited[0] = True
        ...
        # Check for rate limiting and raise specific error
        if rate_limited[0]:
