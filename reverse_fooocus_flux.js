...
async function reverseEngineerFooocusFlux() {
    console.log('🔍 Starting Fooocus FLUX Reverse Engineering\n');
    ...
    try {
        ...
        page.on('response', async response => {
            ...
            if ((url.includes('/api/') || url.includes('generate') || url.includes('image') || url.includes('flux')) && 
        ...
        const pageInfo = await page.evaluate(() => {
            return {
                ...
                gradioComponents: {
                    blocks: document.querySelectorAll('[class*="gradio"]').length,
                    blocksContainer: document.querySelectorAll('.gradio-container').length,
                    app: document.querySelectorAll('#__next').length
                }
            };
        });
        ...
        console.log(`   Gradio Components: ${pageInfo.gradioComponents.blocks}\n`);
        ...
        const outputDir = path.join(__dirname, 'fooocus_flux_analysis');
        ...
        // Generate report
        const report = `# Fooocus FLUX - Reverse Engineering Report

## Analysis Information
- **Date**: ${timestamp}
- **Target**: https://fooocus-one-eight.vercel.app/apps/flux
- **Platform**: Vercel
- **Framework**: Likely Gradio
...
## Technical Stack
- Platform: Vercel
- Framework: Gradio (likely)
- Backend: Fooocus/FLUX
- UI: Next.js + Gradio components

## Next Steps
1. Test discovered API endpoints directly
2. Replicate exact request format (Gradio uses specific formats)
3. Check authentication requirements
4. Document rate limits

---
*Generated by Fooocus FLUX Reverse Engineering Tool*
`;
...
reverseEngineerFooocusFlux()
    .then(data => {
        console.log('\n✅ Reverse engineering completed successfully!\n');
        console.log('Check fooocus_flux_analysis/ folder for complete results.\n');
    })
    .catch(console.error);
