/**
 * Analyze Let's Enhance Background Removal
 * https://letsenhance.io/background-removal
 */

const puppeteer = require('puppeteer');
const fs = require('fs');

async function analyzeLetsEnhance() {
  console.log('🔍 Analyzing Lets Enhance Background Removal...\n');
  ...
  page.on('request', request => {
    ...
    if ((url.includes('api') || url.includes('upload') || url.includes('process') || url.includes('remove') || url.includes('bg') || url.includes('enhance')) && 
        !url.includes('google') && 
  ...
  if (apiRequests.length > 0) {
    ...
    // Save to file
    fs.writeFileSync('letsenhance-captured-requests.json', JSON.stringify(apiRequests, null, 2));
    console.log('💾 Saved to: letsenhance-captured-requests.json\n');
  } else {
...
analyzeLetsEnhance().catch(console.error);
