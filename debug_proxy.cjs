...
// Proxy middleware for the RDP API - with debug logging
const rdpProxy = createProxyMiddleware({
  target: 'https://htmlgw3.apponfly.com',
  changeOrigin: true,
  logLevel: 'debug',
  pathRewrite: {
    '^/rdp-api/(.*)': '/rdp/api/$1' // Match /rdp-api/anything and replace with /rdp/api/anything
  },
  onProxyReq: (proxyReq, req, res) => {
    console.log('Proxying request:', req.method, req.url);
    console.log('Target URL would be:', `https://htmlgw3.apponfly.com${req.url.replace('/rdp-api/', '/rdp/api/')}`);
