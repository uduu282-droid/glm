...
async function testRedFlipBananaAPI() {
    console.log('🧪 Testing Red Flip Banana API: flip-banana.vercel.app');
    console.log('==================================================\n');
    
    const baseUrl = 'https://flip-banana.vercel.app';
    const testPrompt = 'banana';
    
    // Test the generate endpoint with GET request
    const endpoint = {
...
// Run the test
testRedFlipBananaAPI().then(result => {
    console.log('\n' + '='.repeat(50));
    if (result.success) {
        console.log('🎉 Red Flip Banana API Test: SUCCESS');
        if (result.data) {
            console.log('Response Data:', JSON.stringify(result.data, null, 2));
        }
    } else {
        console.log('❌ Red Flip Banana API Test: FAILED');
