"""
GLM Multi-Model Client - Updated to support multiple GLM models with feature controls
Supports: glm-5, glm-4.7, glm-4.6, glm-4.5 with web search and thinking mode
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Chat session - UPDATED FOR MULTI-MODEL SUPPORT
# ─────────────────────────────────────────────────────────────────────────────
...
class ChatSession:
    ...
    def start(self, first_message: str, model: str = None, features: dict = None) -> None:
        ...
        self.history_messages = {
            first_msg_id: {
                ...
                "timestamp": ts_s, "models": [self.model],
            }
        }
        self.history_current_id = first_msg_id

        payload = {"chat": {
            "id": "", "title": "New Chat", "models": [self.model], "params": {},
            "history": {
            ...
            "mcp_servers": [], 
            "enable_thinking": self.features.get("enable_thinking", True), 
            "auto_web_search": self.features.get("auto_web_search", False),
        ...
        print(f"  chat_id = {self.chat_id} (model: {self.model})")
    ...
    def send(self, message: str, features: dict = None) -> str:
    ...
    def _complete(self, message: str, user_msg_id: str, parent_comp_id: str | None) -> str:
        ...
        payload = {
            "stream": True, 
            "model": self.model,  # ✅ Dynamic model
            "messages": [{"role": "user", "content": message}],  # only current message
            "signature_prompt": message,
            "params": {}, "extra": {},
            "features": {  # ✅ Dynamic features
                "image_generation": self.features.get("image_generation", False),
                "web_search": self.features.get("web_search", False),
                "auto_web_search": self.features.get("auto_web_search", False),
                "preview_mode": True,
                "flags": [],
                "enable_thinking": self.features.get("enable_thinking", True),
            ...
            while "\n" in buf[0]:
                ...
                if ph != phase[0]:
                    if ph == "thinking":
                        ...
                        print("\n\033[92m  {} ›\033[0m ".format(self.model), end="", flush=True)
                    elif ph == "other":
                        u = data.get("usage", {})
        ...
        self.history_messages[asst_id] = {
            ...
            "timestamp": _s(), "models": [self.model],
        }
...
def main():
    print(f"z.ai GLM multi-model client  (secret: {_SECRET[:10]}…)\n")
    ...
    if single:
        # ── one-shot ──────────────────────────────────────────────────────────
        print(f"\n[3] Starting chat with {auth['email']} …")
