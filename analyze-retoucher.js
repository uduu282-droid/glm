/**
 * Analyze Retoucher.online Background Removal Service
...
async function analyzeRetoucher() {
  console.log('🔍 Analyzing Retoucher.online...\n');
  ...
  page.on('request', request => {
    ...
    if ((url.includes('api') || url.includes('upload') || url.includes('process') || url.includes('remove')) && 
        !url.includes('google') && 
  ...
  console.log('\n🌐 Navigating to Retoucher.online...\n');
  
  try {
    await page.goto('https://retoucher.online/', {
  ...
  if (apiRequests.length > 0) {
    ...
    // Save to file
    fs.writeFileSync('retoucher-captured-requests.json', JSON.stringify(apiRequests, null, 2));
    console.log('💾 Saved to: retoucher-captured-requests.json\n');
  } else {
...
analyzeRetoucher().catch(console.error);
