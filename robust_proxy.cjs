...
// Proxy middleware for the RDP API - handles all /rdp-api requests
const rdpProxy = createProxyMiddleware({
  target: 'https://htmlgw3.apponfly.com',
  changeOrigin: true,
  // Custom path rewrite function to handle the mapping
  pathRewrite: function (path, req) {
    // Replace /rdp-api/ with /rdp/api/ 
    return path.replace(/^\/rdp-api\//, '/rdp/api/');
  },
