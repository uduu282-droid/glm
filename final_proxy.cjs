...
// Proxy endpoint for RDP API - using router to capture the rest of the path
const rdpApiRouter = express.Router();

rdpApiRouter.use('/', (req, res) => {
  console.log('Received request:', req.method, req.originalUrl);
  
  // Extract the path after /rdp-api/ and convert to /rdp/api/
  // Original URL is like /rdp-api/session/data/...
  // So we replace /rdp-api with /rdp/api
  let apiPath = req.originalUrl.replace('/rdp-api', '/rdp/api');
  ...
  // If there's request body data, send it
