...
async function testImageGenerationAPI() {
    ...
    console.log('🎨 Testing Image Generation API with enhanced headers');
    console.log('URL:', url);
    ...
    try {
        console.log('\n🚀 Sending request with enhanced headers...');
        ...
        if (error.response) {
            ...
            if (error.response.status === 403) {
                if (error.response.data && typeof error.response.data === 'string' && error.response.data.includes('Cloudflare')) {
                    ...
                    console.log('This API requires browser-like behavior or cookies to work');
                } else {
