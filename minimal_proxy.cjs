...
// Proxy endpoint for RDP API
app.get('*', (req, res) => {
  // Check if the request is for our proxy endpoint
  if (req.path.startsWith('/rdp-api')) {
    ...
    // For non-proxy requests, return a simple response
    if (req.path === '/test') {
      res.json({ 
      ...
      res.send('Simple RDP API Proxy Server - Use /rdp-api/* to access the RDP service');
    }
  }
});
